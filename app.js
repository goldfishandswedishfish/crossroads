...
function extractJSON(text) {
  ...
  // Collapse literal newlines/tabs — LLMs sometimes insert them inside string values,
  // which is invalid JSON even though it looks fine in the console
  text = text.replace(/[\r\n\t]+/g, ' ');

  // Try the whole array as-is
  const aStart = text.indexOf('[');
  const aEnd   = text.lastIndexOf(']');
  if (aStart !== -1 && aEnd > aStart) {
    try { return JSON.parse(text.slice(aStart, aEnd + 1)); }
    catch (e) { console.log('[Crossroads] array parse error:', e.message); }
  }
  ...
  // Brace-match fallback: pull out each top-level {...} regardless of surrounding noise
  const objects = [];
  let depth = 0, start = -1;
  ...
  console.log('[Crossroads] all parse strategies failed');
  return null;
}
